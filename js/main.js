...
// 用于渲染模板字符串时对用户输入做基本转义，降低 XSS 风险。
...
// 仅保留电话号码常用字符，避免拼接到 `tel:`/HTML 属性时被注入。
...
async function getApprovedListingsFromDB() {
    try {
        ...
        console.log('房源数据:', JSON.stringify(data));
        return data || [];
    } catch (err) {
...
async function getApprovedListings() {
    ...
    if (!cachedListings || (now - lastFetchTime) > CACHE_DURATION) {
        ...
        // 跨设备共享：展示以云端为准。
        // 只有在云端取不到数据（开发/网络异常）时才回退到本地，避免“本地有/云端没有”导致的假数据。
    ...
    return '<article class="listing-card fade-in">' +
        ...
        '<div class="listing-content">' +
            ...
            '<div class="listing-meta">' +
                ...
                '<span><i class="fas fa-ruler-vertical"></i> ' + escapeHtml(listing.height || 6) + '米</span>' +
...
async function renderDetailPage(listingId) {
    ...
    document.getElementById('detail-info').innerHTML = 
        ...
        '<div class="info-item"><label>层高</label><span>' + escapeHtml(listing.height || '6') + '米</span></div>' +
